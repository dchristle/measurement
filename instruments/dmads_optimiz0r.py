...
#        plotff = qt.plot(self._fed, coorddim=0, valdim=1, maxtraces=1, name='fevalp', clear=True)
    ...
    def iterate(self):
        ...
        if np.array(self._x0).size > 1 and self._complete_poll == True:
            ...
            self._hypd.add_data_point(self._iteration_number, T4, q_crit, self._sample_size, self._cur_f, self._nfeval, np.log(np.min(self._scale)/np.max(self._scale))/np.log(10))
            ...
            self._hypd.add_data_point(self._iteration_number, self._cur_f, self._nfeval, np.log(np.min(self._scale)/np.max(self._scale))/np.log(10))
            self._fed.add_data_point(self._nfeval, np.log(np.abs(self._cur_f + 600)))
        ...
        if min_f < self._cur_f:
            ...
            if self._method == 'dynamic_mads':
                ...
                # now scale the successful directions element by element with
                # the beta vector
                ...
                # if not using dynamic mads, scale every direction by a scalar
            ...
            if self._method == 'dynamic_mads':
                # here, the shrinking respects the choice of beta vector
                # and scales element by element instead of by just a scalar
                self._scale = self._scale/self._beta
            else:
                self._mesh_size = self._mesh_size*self._mesh_shrinkage_factor
            self._history.append(self._cur_f)
    ...
    def _gps2nbasis(self, x):
        ...
        if self._method == 'dynamic_mads':
            # generate a dense set of directions by picking a random point on the
            # unit sphere each and then computing the Householder matrix.
            # this matrix is then scaled by the current mesh scaling vector,
            # and then rounded to an integer to give a valid polling point.
            ...
            dirMat = np.round(Hm*(self._delta/self._scale))*self._scale
            print 'dir mat %s' % dirMat
            # like the standard gps search, construct the 2N basis by taking the
            # chosen directions and appending their negatives
        ...
        if self._method == 'dynamic_mads':
            # for dmads, we ignore the mesh_size parameter -- a bit of a hack
