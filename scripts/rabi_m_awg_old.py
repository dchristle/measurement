...
class SiC_Rabi_Master(m2.Measurement):
    ...
    def sequence(self):
        ...
        for i in range(self.params['pts']):
            ...
            e.add(pulse.cp(sq_pulseAOM, amplitude=1, length=1.8e-6), name='laser init')
            e.add(pulse.cp(sq_delay, amplitude=0.0, length=0.8e-6), name='delay1', refpulse='laser init', refpoint='end')
            #e.add(pulse.cp(sq_pulseMW, amplitude=1, length=0.6e-6), name='microwave pulse', refpulse='delay1', refpoint='end')
            e.add(pulse.cp(sq_pulseMW, length = self.params['MW_pulse_durations'][i], amplitude = 1.0), name='microwave pulse', refpulse='delay1', refpoint='end')
            e.add(pulse.cp(sq_delay, amplitude=0.0, length=0.3e-6),
        	name='delay2', refpulse='microwave pulse', refpoint='end')
            e.add(pulse.cp(sq_pulsePC, amplitude=1.0, length=0.21e-6, time_offset=400e-9),
