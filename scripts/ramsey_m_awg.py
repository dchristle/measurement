...
class SiC_Ramsey_Master(m2.Measurement):
    ...
    def sequence(self, upload=True, program=True, clear=False):
        ...
        for i in range(self.params['pts']):
            ...
            e = element.Element('ElectronRamsey_pos_pt-%d' % i, pulsar=qt.pulsar)
    ...
    def measure(self):
        ...
        print '--Ramsey meas. from %.4f ns to %.4f ns in %.4f ns steps (%.2f steps), %.5f GHz --' % (self.params['tau_length_start'], self.params['tau_length_end'], self.params['tau_length_step'], self.params['pts'], self.params['freq'])
