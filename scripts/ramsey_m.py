...
class SiC_Ramsey_Master(m2.Measurement):
    ...
    def prepare(self):
        ...
        time.sleep(self._ddgsleep)
        # Set the first pi/2 switching delay

        self._ddg.set_delayC(self.params['A_delay']*1.0e-9 + self.params['AOM_length']*1.0e-9 + self.params['RF_delay']*1.0e-9)
        time.sleep(self._ddgsleep)
        ...
        time.sleep(self._ddgsleep)
        self._ddg.set_delayD(self.params['RF_pi2']*1.0e-9)
        time.sleep(self._ddgsleep)

        # Set the second pi/2 switching delay

        self._ddg.set_delayE(self.params['A_delay']*1.0e-9 + self.params['AOM_length']*1.0e-9 + self.params['RF_delay']*1.0e-9 + self.params['RF_pi2']*1.0e-9)
        time.sleep(self._ddgsleep)
        self._ddg.set_polarityEF('pos')
        time.sleep(self._ddgsleep)
        self._ddg.set_offsetEF(0.0)
        time.sleep(self._ddgsleep)
        self._ddg.set_amplitudeEF(self.params['RF_amplitude'])
        time.sleep(self._ddgsleep)
        self._ddg.set_delayF(self.params['RF_pi2']*1.0e-9)
        time.sleep(self._ddgsleep)

        # Set the switch photon readout delay

        self._ddg.set_delayG(self.params['A_delay']*1.0e-9 + self.params['AOM_length']*1.0e-9 + self.params['RF_delay']*1.0e-9 + self.params['RF_pi2']*1.0e-9)
        time.sleep(self._ddgsleep)
        self._ddg.set_polarityGH('pos')
        time.sleep(self._ddgsleep)
        self._ddg.set_offsetGH(0.0)
        time.sleep(self._ddgsleep)
        self._ddg.set_amplitudeGH(self.params['switch_amplitude'])
        time.sleep(self._ddgsleep)
    ...
    def measure(self):
        ...
        print '--Ramsey meas. from %.4f ns to %.4f ns in %.4f ns steps (%.2f steps)--' % (self.params['delay_start'], self.params['delay_end'], self.params['delay_step'], n_steps)
        ...
        for i in range(self.params['MeasCycles']):
            ...
            plot2d_0 = qt.Plot2D(total_count_data, name='ramsey_single_sweep', clear=True)
            plot2d_1 = qt.Plot2D(total_count_data, name='ramsey_avg', clear=True)
            ...
            for j in range(n_steps):
                ...
                # Set the new second RF pi/2 pulse delay
                self._ddg.set_delayE(self.params['A_delay']*1.0e-9 + self.params['AOM_length']*1.0e-9 + self.params['RF_delay']*1.0e-9 + self.params['RF_pi2']*1.0e-9 + delays_temp[j]*1.0e-9) # units of ns
                time.sleep(self._ddgsleep)
...
xsettings = {
        ...
        'switch_length' : 400.0, # ns
        'switch_delay' : 1000.0, # ns
        'ctr_term' : 'PFI0',
        'power' : -57.0, # dbM
        'delay_start' : 0.0, # ns
        'delay_end' : 400.0, # ns
