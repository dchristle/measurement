...
class SiC_PL6PHStanda_Master(m2.Measurement):
    ...
    def measure(self):
        ...
        n_wls = self.params['step'].size
        if self.params['cycle_waveforms']:
            n_wfms = self.params['waveform_list'].size
            average_s_data = np.zeros((n_wfms*n_wls, 65536), dtype='uint32')
            print 'Created array for %d waveforms at %d steps.' % (n_wfms, n_wls)
        else:
            print 'Running with existing waveform -- will not cycle. Just steps.'
            n_wfms = 1
        ...
        for i in np.arange(self.params['MeasCycles']):
            for k in np.arange(n_wls):
                self._st0.move(int(self.params['step'][k]))
                qt.msleep(0.5)
                n = 0
                while n < 50:
                    cur_pos = self._st0.get_position()
                    if int(cur_pos) == int(self.params['step'][k]):
                        break
                    else:
                        n = n + 1
                        qt.msleep(0.5)

                print 'Standa set to %.3f, indicated step is %.3f' % (self.params['step'][k], cur_pos)
                for j in np.arange(n_wfms):
                    ...
                    if self.params['cycle_waveforms']:
                        ...
                        print 'Waveform index %d -- waveform %d/%d, step index %d/%d.' % (int(self.params['waveform_list'][j]), j+1, n_wfms, k+1, n_wls)
...
xsettings = {
        ...
        'waveform_list' : np.array((1,3)),
        'ctr_term' : 'PFI0',
        'step' : np.array((-8000,-12000)), # nm
