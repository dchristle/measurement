...
class SiC_PL6PH_Master(m2.Measurement):
    ...
    def prepare(self):
        ...
        time.sleep(0.2)
        self._mc.set_wavelength(self.params['wavelength'])
        current_wl = self._mc.get_wavelength()
        if np.abs(current_wl - self.params['wavelength']) < 1.0:
    ...
    def measure(self):
        ...
        if self.params['cycle_waveforms']:
            ...
            average_s_data = np.zeros((n_wfms, 65536), dtype='uint32')
            print 'Created array for %d waveforms.' % n_wfms
        else:
            print 'Running with existing waveform -- will not cycle.'
            n_wfms = 1
        ...
        for i in np.arange(self.params['MeasCycles']):
            for j in np.arange(n_wfms):
                ...
                if self.params['cycle_waveforms']:
                    ...
                    print 'Waveform index %d -- waveform %d/%d.' % (int(self.params['waveform_list'][j]), j+1, n_wfms)
                ...
                if self.params['cycle_waveforms'] and j == n_wfms-1:
                    plot2dlog1 = qt.Plot2D(np.log(1.0+np.double(average_s_data[2,:])), name='sicpl6ph_logarithmend', clear=True)
                if not self.params['cycle_waveforms']:
                    plot2dlog = qt.Plot2D(np.log(1.0+np.double(average_s_data)), name='sicpl6ph_logarithm', clear=True)
