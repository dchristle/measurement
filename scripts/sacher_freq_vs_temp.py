...
temp_vec = np.linspace(21.75,23.75,30)
schr = qt.instruments['schr']
fp = qt.instruments['fp']
schr.set_temperature(temp_vec[0])
...
while n < 12:
        ...
        print 'Measured temperature %.3f, desired %.3f' % (t_actual, temp_vec[0])
        if np.abs(temp_vec[0] - t_actual) < 0.03:
            time.sleep(10.0)
            t_actual = schr.get_temperature()
            if np.abs(temp_vec[0] - t_actual) < 0.03:
