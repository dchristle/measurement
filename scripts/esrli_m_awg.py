...
class SiC_ESRLI_Master(m2.Measurement):
    ...
    def prepare(self):
        ...
        self._li = qt.instruments['lockin']
        ...
        elif desired_atten < 0.0:
            self._va.set_attenuation(0.0)
            print 'Desired attenuation is less than zero, setting attenuation to 0.0 dB'
        else:
    ...
    def measure(self):
        ...
        total_count_data = np.zeros((2,n_steps), dtype='float')
        # Make an empty array to store tha average femtowatt data in
        average_count_data = np.zeros((2,n_steps), dtype='float')
        ...
        mm = 0
        kk = 0
        while kk < 22 and mm < 3:

            time.sleep(5.0)
            current_temp = self._ls332.get_kelvinA()
            if np.abs(current_temp-current_setpoint) < 0.25:
                mm = mm + 1
            else:
                mm = 0
            kk = kk+1
            print 'Stabilization loop index is %d, %.2f K' % (kk, current_temp)
            if kk == 21:
        ...
        if self.keystroke('abort') in ['q','Q']:
            ...
            return
        # Set locking TC
        self._li.set_TC(int(self.params['time_constant']))
        # Get TC in seconds
        self._tc = self._li.get_TC()
        # Create two data structures and two plot objects for plotting
        data = qt.Data(name='esr_single')

        data.add_coordinate('frequency (ghz)')
        data.add_value('X')
        data.add_value('Y')

        data.add_data_point(np.zeros((n_steps,3), dtype='float'))
        plot2d_0 = qt.Plot2D(data, coorddim=0, valdim=1, name='esr_single_sweep')
        #plot2d_0.add_data(data,coorddim=0,valdim=2)
        dataa = qt.Data(name='esr_avg')

        dataa.add_coordinate('frequency (ghz)')
        dataa.add_value('X')
        dataa.add_value('Y')

        dataa.add_data_point(np.zeros((n_steps,3), dtype='float'))
        plot2d_1 = qt.Plot2D(dataa, coorddim=0, valdim=1, name='esr_avg')
        #plot2d_1.add_data(dataa,coorddim=0,valdim=2)
        ...
        for i in range(self.params['MeasCycles']):
            ...
            sorted_count_data = np.zeros((2,n_steps), dtype='float')
            temp_count_data = np.zeros((2,n_steps), dtype='float')
            ...
            for j in range(n_steps):
                ...
                if setVal != 0:
                    ...
                    break
                # Wait, so lockin filter can accumulate signal
                time.sleep(self._tc*self.params['time_constant_multiple'])

                cur_x, cur_y = self._li.get_XY()
                temp_count_data[0,j] = cur_x
                temp_count_data[1,j] = cur_y

                #data.add_data_point(freq_temp[j],temp_count_data[j])


            tt = time.time() - t1
            print 'Total time is %.3f, efficiency of %.2f percent. Heater output at %.1f.' % (tt, (n_steps*self._tc*self.params['time_constant_multiple'])/tt*100.0, self._ls332.get_heater_output())
...
##                data.add_data_point(freq[i],sorted_temp_data[0,i],sorted_temp_data[1,i])
            pltary = np.zeros((n_steps,3), dtype='float')
            pltary[:,0] = freq
            pltary[:,1] = sorted_temp_data[0,:]
            pltary[:,2] = sorted_temp_data[1,:]


            plot2d_0.clear()
            plot2d_0.add(pltary[:,(0,1)])
            plot2d_0.add(pltary[:,(0,2)])
            plot2d_0.update()
            ...
            # it was taken at.
            total_count_data = total_count_data + sorted_temp_data
...
##            dataa.add_value('Y')
##            for i in range(n_steps):
##                dataa.add_data_point(freq[i],average_count_data[0,i],average_count_data[1,i])
            pltary = np.zeros((n_steps,3), dtype='float')
            pltary[:,0] = freq
            pltary[:,1] = average_count_data[0,:]
            pltary[:,2] = average_count_data[1,:]


            plot2d_1.clear()
            plot2d_1.add(pltary[:,(0,1)])
            plot2d_1.add(pltary[:,(0,2)])
            #plot2d_1.update()
            #plot2d_1 = qt.Plot2D(dataa, coorddim=0, valdim=0, name='esr_avg')
            gc.collect()
            qt.msleep(0.01) # keeps GUI responsive and checks if plot needs updating.
        ...
        grp.add('counts', data=total_count_data, unit='counts', note='total XY signal')
        grp.add('avgcounts', data=average_count_data, unit='counts', note='average XY signal per measurement cycle')
...
xsettings = {
        ...
        'time_constant' : 11, # seconds
        'time_constant_multiple' : 3, # decimal
        }
...
for rr in range(p_nstep):
    ...
    # power it's taken at.
    m = SiC_ESRLI_Master(name_string)
